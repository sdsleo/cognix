...
export function CenterSection({
  ...
  const handleInputType = (type: string) => {
    switch (type) {
        ...
        return (
          <>
            <MultSelectCheckbox
              ...
              placeholder="Pesquisar"
              clear={clearSelect}
              className="cnx-advanced-mult-check-box-container-width"
            />
            <button
              type="button"
              className="cnx-btn-save-center-section-cbscs"
      ...
      case "DateRangerPicker":
        return (
          <>
            <DateRangerPicker inputRef={inputRef} />
            <button
              type="button"
              className="cnx-btn-save-center-section-cbscs"
              onClick={() =>
  ...
  const getInputRefValue = (type: any, query: any, title: any) => {
    ...
    if(singleSelects) {
      if (filtersSelected.some((item: any) => item.query === keepSelect)) {
        const newSelected = filtersSelected.filter((item: any) => item.query !== keepSelect)
        setFiltersSelected([...newSelected, newSelectedFilterObject]);
      } else {
  ...
  const getDateRageInputRefValue = (type: any, query: any, title: any) => {
    ...
    if (hasQuery) {
      const temp = filtersSelected?.filter((item: any) => item.query === query);
      const filterOld = [...filtersSelected];
      const objWithIdIndex = filterOld.findIndex((obj: any) => obj.id === temp[0].id);

      if (objWithIdIndex > -1) {
        filterOld.splice(objWithIdIndex, 1);
      }
      ...
      setFiltersSelected([...filterOld, newSelectedFilterObject]);
  ...
  const getInputRefValueCnxInput = (
    ...
    if (isArray) {
      ...
      const selectFilter = singleSelects ? filtersSelected.filter((item: any) => item.query === query || item.query ===  keepSelect) : filtersSelected;

      setFiltersSelected([...selectFilter, ...temp]);
    } else {
      const id = `${title}${getRandomInt(50)}${getRandomInt(100)}`;
      const newSelectedFilterObject = {
        id,
      ...
      const selectFilter = singleSelects ? filtersSelected.filter((item: any) => item.query !== query || item.query === keepSelect) : filtersSelected;
      setFiltersSelected([...selectFilter, newSelectedFilterObject]);
    }
  };
