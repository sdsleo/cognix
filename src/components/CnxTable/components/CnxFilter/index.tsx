...
function CnxFilter({ id, filterResponse, filterOptions, clearFilter, setFilter }: any) {
  ...
  const handleQueryObject = (filters: any) => {
    ...
    const ObjectEntriesQuery = querys.map((query: any) => {
      const objectFiltered = filters.filter((filter: any) => filter.query == query);
  ...
  return (
    ...
    >
      <div className="cnx-filter-modal-cfm">
        ...
        <LeftSection filters={filterOptions?.filters} setCurrentSelect={setCurrentSelect} />
        <CenterSection currentSelect={currentSelect} setFiltersSelected={setFiltersSelected} filtersSelected={filtersSelected} />
        <RightSection currentSelect={currentSelect} filtersSelected={filtersSelected} setFiltersSelected={setFiltersSelected} />
        <footer className="cnx-filter-footer-container-cffc">
          <button type="button" className="cnx-filter-btn-clear-cfbc" onClick={() => handleClearFilter()}>Limpar</button>
          <button type="button" className="cnx-filter-btn-apply-cfba" onClick={() => handleSubmit()}>Aplicar</button>
        </footer>
      </div>
    </div>
  );
